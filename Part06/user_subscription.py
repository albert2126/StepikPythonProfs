...
Дополните приведенный ниже код, чтобы вывел данные о каждом пользователе из этого списка, предварительно отсортировав их по статусу подписки от дорогой к дешевой, а при совпадении статусов — в лексикографическом порядке адресов электронных почт. Данные о каждом пользователе должны быть указаны в следующем формате:
...
def plan_compare(x, y):
    return plans.index(x.plan) - plans.index(y.plan)


sort_users = sorted(sorted(users, key=lambda u: u.email),  key=cmp_to_key(plan_compare), reverse=True)
